 ...
 class q3db {
     ...
     public function updateDB( ) {
         ...
         foreach ( $this->serverlist as $key => $server ) {
             ...
             if ( $status ) {
                 $this->logmsg( "Successfully got server info: $server - " . strip_colors( $this->q3queries[ $server ]->get_cvar( 'sv_hostname' ) ) );
             } //$status
             else {
                 $this->logmsg( "Failed to get server info: $server" );
             }
         } //$this->serverlist as $key => $server
     ...
     public function lookupPlayer( $name ) {
         ...
         foreach ( $this->q3queries as $key => $query ) {
             foreach ( $query->playerlist as $key2 => $player ) {
                 ...
                 } //stripos( $player[ "strippedname" ], $name ) !== false
             } //$query->playerlist as $key2 => $player
         } //$this->q3queries as $key => $query
     ...
     public function lookupServer( $name ) {
         ...
         foreach ( $this->q3queries as $key => $query ) {
             ...
             if ( stripos( $stripedname, $name ) !== false ) {
                 ...
                 $serverlist[ ] = $query;
             } //stripos( $stripedname, $name ) !== false
             else {
