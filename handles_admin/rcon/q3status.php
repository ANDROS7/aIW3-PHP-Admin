 ...
 class q3status {
     ...
     private function get_response( ) {
         ...
         while ( $buff = fread( $this->socket_connection, 9999 ) ) {
             ...
             $buffer .= $contents;
         } //$buff = fread( $this->socket_connection, 9999 )
         return $buffer;
     }
     ...
     public function update_status( ) {
         ...
         for ( $i = 0; $i < count( $playerexplode ); $i++ ) {
             ...
             if ( $playerline != "" ) { // Make sure this isn't an empty line.
                 ...
                 $playerarray[ $i ] = array(
                      'name' => $name,
                     'strippedname' => strip_colors( $name ),
                     'ping' => intval( $ping ),
                     'score' => intval( $score ) 
                 );
             } //$playerline != ""
         } //$i = 0; $i < count( $playerexplode ); $i++
     ...
     public function get_cvar( $name ) {
         ...
         } //array_key_exists( $name, $this->cvarlist )
         else {
             return "";
         }
     }
